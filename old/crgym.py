...
class CryptoEnv(gym.Env):
    ...
    def __init__(self, df, title=None):
        self.df = df
        self.reward_range = (-static.MAX_ACCOUNT_BALANCE,static.MAX_ACCOUNT_BALANCE)
        self.total_fees = 0.0
        self.total_volume_traded = 0.0
        self.crypto_held = 0.0
        self.shib_eur_held = static.SHIBEURHELD
        self.shib_eur_held_start = static.SHIBEURHELD
        self.episode = 1
        ...
        #Action space from -1 (short) to 1 (buy)
        ...
        self.observation_space = spaces.Box(low=0,high=1,shape=(10,5),dtype=np.float16)

    def reset(self):
        self.balance = static.INITIAL_ACCOUNT_BALANCE
        self.net_worth = static.INITIAL_ACCOUNT_BALANCE + static.SHIBEURHELD
        self.max_net_worth = static.INITIAL_ACCOUNT_BALANCE + static.SHIBEURHELD
        self.total_fees = 0.0
        self.total_volume_traded = 0.0
        self.crypto_held = 0.0
        self.shib_eur_held = static.SHIBEURHELD
    ...
    def _next_observation(self):
        ...
        frame = np.array([self.df.loc[self.current_step - 4:self.current_step, 'Open'],
                          ...
                          self.df.loc[self.current_step - 4:self.current_step, 'Quote asset volume'],
                          self.df.loc[self.current_step - 4:self.current_step, 'Number of trades'],
                          self.df.loc[self.current_step - 4:self.current_step, 'Taker buy base asset volume'],
                          self.df.loc[self.current_step - 4:self.current_step, 'Taker buy quote asset volume']])
        ...
        obs = np.append(frame, 
                        [[self.balance /  static.MAX_ACCOUNT_BALANCE,
                          ...
                          self.shib_eur_held / self.shib_eur_held_start,
                          0]],
                        axis=0)
        return obs
    ...
    def _take_action(self, action):
        ...
        current_price = random.uniform(self.df.loc[self.current_step, 'Real open'],self.df.loc[self.current_step, 'Real close'])
        if action[0] > 0:
            # Buy
    ...
    def step(self, action, end=True):
        ...
        profit = self.net_worth - (static.INITIAL_ACCOUNT_BALANCE + static.SHIBEURHELD)
        profit_percent = profit / (static.INITIAL_ACCOUNT_BALANCE + static.SHIBEURHELD) * 100
